...
// Kích hoạt Telemetry Auto-Logger cho toàn bộ API endpoints
  ...
  try {
    ...
    if (hasTerm && !isForcedStats) {
      ...
      try {
        ...
        const candidates = await hybridRetrieve(query, 3, resolvedTenantId);
...
Ensure the reply is beautifully formatted using markdown.`;
        ...
        if (llmOutput && llmOutput.trim().length > 0 && !llmOutput.includes("step_1") && !llmOutput.includes("Mục tiêu")) {
          ...
          responseData.text = llmOutput.trim();
        }
      } catch (err: any) {
...
- Số lượng nhân viên dưới trướng: ${employeesCount} người.
...
`;
    ...
    if (replyText.includes("[ĐỒNG Ý MẶC CẢ]")) {
      ...
      // Extract proposed price from user message to execute database update
      if (chatHistory && chatHistory.length > 0) {
        const lastMsg = chatHistory[chatHistory.length - 1];
    ...
    return c.json({ success: true, text: replyText });
  } catch (err: any) {
...
app.post("/agent/sabotage", async (c: any) => {
  try {
    ...
    if (chosenType === "rumor") {
      ...
      publicAnnouncement = `⚠️ [Cạnh tranh thị trường] Agent ${attacker.name} tung tin đồn sản phẩm của Agent ${victim.name} không đạt chuẩn VietGAP, khiến đối thủ sụt giảm 15% và thiệt hại 2M ngân sách!`;
      ...
      publicAnnouncement = `⚡ [Phá giá thị trường] Agent ${attacker.name} tung chương trình khuyến mãi chớp nhoáng, cướp đoạt trực tiếp 5,000,000đ doanh thu từ giỏ hàng của Agent ${victim.name}!`;
      ...
      publicAnnouncement = `🚨 [Tố cáo sai phạm] Agent ${attacker.name} đệ đơn tố cáo Agent ${victim.name} đóng gói sai quy chuẩn lên Ban Quản Lý, khiến đối thủ bị Trưởng ban ${leaderName} phạt hành chính 3,000,000đ!`;
    } else {
      // Chèn ép logistics
      const attackerOrigBudget = Number(attackerProfile.budget);
      const attackerBase = Number.isFinite(attackerOrigBudget) ? attackerOrigBudget : (serverAgentBudgets[attacker.id] ?? 0);
      ...
      publicAnnouncement = `📦 [Chèn ép logistics] Agent ${attacker.name} áp đặt thành công biểu phí trung chuyển mới lên Agent ${victim.name}, thu lợi 3,000,000đ phí chênh lệch vận tải!`;
    }
