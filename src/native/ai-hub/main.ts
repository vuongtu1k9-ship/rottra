...
function generate_image(prompt: string): void {
  const mock_file = `bin/output/image_${Math.floor(Math.random() * 1000)}.avif`;
...
function generate_video(prompt: string): void {
  const mock_file = `bin/output/video_${Math.floor(Math.random() * 1000)}.mp4`;
  console.log(JSON.stringify({
    status: "success",
    type: "video",
    file_path: mock_file,
...
function generate_audio(prompt: string): void {
  const mock_file = `bin/output/audio_${Math.floor(Math.random() * 1000)}.wav`;
  console.log(JSON.stringify({
    status: "success",
    type: "audio",
    file_path: mock_file,
